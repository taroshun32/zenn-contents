...
Working Memoryは、通常の会話履歴（lastMessages）とは別に管理される、**AIがユーザーについて覚えておくべき情報を永続的に記録する機能**です。スコープ設定により、スレッド単位またはリソース横断的に記憶を保持できます。例えば「私は田中といいます」と伝えると、AIが自動的にその情報を保存し、次回以降の会話でも「田中さん」と呼びかけられるようになります。
...
Semantic Recallは、**AIが関連する過去の会話を検索して思い出す機能**です。「前に話した〇〇」のような曖昧な言及でも、AIが通常の会話履歴（lastMessages）から外れた古い会話履歴から関連する内容を見つけ出します。
